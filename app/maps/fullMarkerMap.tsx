...
const Page: React.FC = () => {
  ...
  const fetchLibraries = async () => {
    ...
    try {
      ...
      const allData = responses.flatMap(
        (response) => response.data.SeoulLibraryTimeInfo.row
      );
  ...
  const fetchMuseums = async () => {
    const url =
      'https://datasets-server.huggingface.co/rows?dataset=hscrown%2Fseoul_museums&config=default&split=train&offset=0&length=100';
    try {
      const response = await axios.get(url);
      const data = response.data;
      if (data && data.rows) {
        const cleanedData = data.rows
          .map((item: any) => ({
            name: item.row['시설명'],
